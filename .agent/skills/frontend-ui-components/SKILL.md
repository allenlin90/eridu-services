...
| Loading / `Suspense` fallback | Use the shared `LoadingPage` / `LoadingSpinner` from `@eridu/ui` (pass `label`/`className`) — don't hand-roll loading divs. A code-split `Suspense` fallback must stay in the **eager** shell to render during its child's load, so keep it a dependency-free spinner — **don't add Lottie/heavy animation libs to a fallback** (it bloats the bundle the split is shrinking). |
