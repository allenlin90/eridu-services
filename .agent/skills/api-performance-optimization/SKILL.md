...
- **Don't drop columns a sibling consumer still needs inline.** Trimming a heavy field (e.g. a JSONB `schema`) from a shared list `include` is only safe if every consumer re-fetches it via a detail endpoint. A list feeding an inline-render view must keep the field (see `taskListIncludeWithSchema`).
