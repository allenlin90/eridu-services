...
export const schedulePublishSummarySchema = z
  .object({
    ...
    shows_preserved: z.number().int().nonnegative(),
    shows_skipped: z.number().int().nonnegative(),
    confirmed_shows_updated: z.number().int().nonnegative(),
    confirmed_shows_pending_resolution: z.number().int().nonnegative(),
    publish_impacts_recorded: z.number().int().nonnegative(),
    ...
    creator_mappings_backfilled: z.number().int().nonnegative(),
  })
  .pipe(
    z.object({
      shows_created: z.number().int().nonnegative(),
