...
1. **Build shared translations** (must be done before using in apps):
   ```bash
   pnpm turbo run build --filter=@eridu/i18n
   ```
   Two turbo tasks run in order. `generate` compiles translations from `messages/{lang}.json` into `src/paraglide/`; `build` then type-compiles that into `dist/`:
   - `src/paraglide/` - Generated Paraglide runtime files (produced by `generate`)
   - `dist/` - Built package for distribution, included in exports (produced by `build`)

   Run them through turbo rather than `cd packages/i18n && pnpm build`. The package's `build` script no longer invokes the Paraglide compile itself — turbo owns that ordering so the compile can never overwrite `src/paraglide/` while another task is reading it. `pnpm install` also runs `generate` via the `prepare` hook, so a fresh clone already has `src/paraglide/`.
...
```bash
# Compile messages into src/paraglide/ only
pnpm --filter @eridu/i18n generate

# Compile messages, then build dist/
pnpm turbo run build --filter=@eridu/i18n

# Watch mode for development (auto-recompiles on changes)
pnpm --filter @eridu/i18n dev
```
...
2. Run `pnpm turbo run build --filter=@eridu/i18n` from the repository root
...
5. **Rebuild packages** (from the repository root):
   ```bash
   pnpm turbo run build --filter=@eridu/i18n
...
- **`paraglide/`**: Auto-generated by Paraglide — in this package by the `generate` task, in each app by the compile step at the front of its own `build`/`typecheck`/`dev` script
...
4. Ensure both packages are built: `pnpm build` from the repository root

### Type errors

Make sure both packages are built, from the repository root:
