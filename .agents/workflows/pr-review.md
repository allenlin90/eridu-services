...
- [ ] **If the PR changes a data model, cross-service/transactional workflow, or module boundary**: the PR description embeds Mermaid diagrams illustrating the change — an ERD for new/changed entities and relations, a sequence diagram for the runtime flow (including the transaction boundary and any rollback/error branch), and a flowchart for branching decision logic (state transitions, signal-to-outcome rules). Pick the subset that actually applies; a single-entity CRUD addition may only need the ERD. The same diagrams belong in the durable doc this PR updates (design/feature doc, skill), not only the PR description — see `doc-hygiene`'s diagram guidance. A PR that only touches a single function's internals, a config value, or a copy change may skip this.
- [ ] **If the PR builds on an existing implementation rather than adding a net-new feature**: the description states what already existed vs. what this PR changes or adds, and links back to the prior PR(s)/commit(s) or existing code that established the baseline. For diagrams under the previous item, mark what's new (e.g. a `==>|new|` edge or a before/after pair) rather than presenting the post-change state as if it had no history — a reviewer who only sees the end state can't tell which parts are actually under review.
