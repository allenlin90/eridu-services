...
**Skill:** `.agents/skills/prod-data-sync/SKILL.md`
...
- **Prod connection timeout / proxy close** — follow `.agents/skills/prod-data-sync/SKILL.md` § Failure Triage before retrying. Confirm prod reachability, try schema-only `pg_dump` if needed, then retry the standard sync once. Ask before using partial or table-excluded restores.
