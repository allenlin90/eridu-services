...
- Keep durable policy in Git; use deployed UIs for monitoring, debugging, and pilot-only changes. Scoped, recorded exception: the `creator-services-tiktok-shop` knowledge collection's access grants are currently set manually in Open WebUI rather than derived from Git frontmatter (owner `erisa-creator-services`; gate and trigger to close it in `ai/architecture/llm-knowledge-base-plan.md` § Content Contract). Its exact approved group/permission set lives in `ai/openwebui/access/audience-group-map.json` and must match the live collection before and after every upload. This rule stays mandatory everywhere else, and for that collection once the gate lifts.
