...
2. For any status write, read § Status write paths and gate invariants first. Only `ShowCancellationGateService` may move a show into or out of `CANCELLED` / `CANCELLED_PENDING_RESOLUTION`.
