...
Use this skill before applying the service-pattern, repository-pattern, or
orchestration-service skills to refactoring work. This skill determines which
lower-level pattern, if any, is appropriate. Existing lower-level skills remain
implementation references where this skill selects their pattern.
...
The existing architecture guide identifies the main structural problem:

- `models/` largely mirrors database tables;
- audience folders such as `admin/`, `studios/`, and `me/` organize transport callers;
- orchestration folders coordinate workflows without a clear capability owner;
...
- repositories are sometimes private persistence and sometimes exported directly.
...
- focused internal orchestration where it improves cohesion;
...
Lower-level implementation references:
