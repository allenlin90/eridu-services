...
3. **A deeper root cause was found but deliberately left for a follow-up.** `BaseRepository` bound inherited base methods to the unbounded client, so the earlier PR kept its workaround and logged the generic repair separately. T9 later added a transaction-aware lazy delegate and real-database rollback coverage. **Lesson: defer a cross-cutting root fix when the current PR cannot characterize it safely, then close the recorded debt with dedicated evidence.**
