...
This is a **process skill**: it tells you *how* to sequence and verify the work. Pair it with the implementation skills for the app you're hardening (backend: `nestjs-architecture`, `backend-large-file-refactor`; frontend: `code-quality`, `frontend-api-layer`, `frontend-state-management`, `frontend-ui-components`, `frontend-testing-patterns`).
...
3. Map the theme equivalents: god-files → large route components (>200 LOC, see `code-quality` § frontend quality); `Prisma/Decimal` leakage → untyped API responses / `any` in the api-layer (`frontend-api-layer`); transaction wiring → query/mutation + cache-invalidation correctness (`frontend-state-management`); convention drift → component/file naming, form contracts, refresh/pagination patterns (`frontend-ui-components`); persisted-JSON casts → API DTO parsing at the boundary.
