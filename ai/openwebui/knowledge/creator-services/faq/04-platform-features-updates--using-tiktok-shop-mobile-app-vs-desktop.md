---
id: creator-services.faq.platform-features-updates--using-tiktok-shop-mobile-app-vs-desktop
title: Platform Features & Updates — Using TikTok Shop Mobile App vs. Desktop
audiences: [erisa]
owner: erisa-creator-services
sensitivity: department
status: active
source_refs: [CS_TikTok_Shop__Knowledge_Base.xlsx]
reviewed_at: 2026-07-23
review_by: 2026-10-21
---

# Platform Features & Updates — Using TikTok Shop Mobile App vs. Desktop

> ฐานความรู้สำหรับผู้ช่วย Creator Services (ERISA) | หมวด: Platform Features & Updates / Using TikTok Shop Mobile App vs. Desktop
> Knowledge base for the ERISA creator-service assistant. Category: Platform Features & Updates / Using TikTok Shop Mobile App vs. Desktop.
> Audience: TikTok Shop creators (Thailand). Source: CS TikTok Shop Knowledge Base.

## Q 3.1.2: คู่มือ TikTok สำหรับเดสก์ท็อปสำหรับสตรีมเมอร์
_EN: TikTok Desktop Guide for Streamers_

ศึกษารายละเอียดจากคู่มืออย่างเป็นทางการของ TikTok Shop Academy:
https://seller-th.tiktok.com/university/essay?identity=1&role=1&knowledge_id=6837891779528449&from=other&sourceType=recommend

_(หมายเหตุสำหรับผู้ช่วย AI: เนื้อหาบทความนี้ยังไม่ได้นำเข้าฐานความรู้ ให้ส่งลิงก์นี้ให้ครีเอเตอร์โดยตรง)_

---

## Q 3.1.3: การใช้ซอฟต์แวร์ OBS สำหรับไลฟ์สตรีม
_EN: Using OBS software for live streaming._

ศึกษารายละเอียดจากคู่มืออย่างเป็นทางการของ TikTok Shop Academy:
https://seller-th.tiktok.com/university/essay?identity=1&role=1&knowledge_id=6837901778880258&from=other&sourceType=recommend

_(หมายเหตุสำหรับผู้ช่วย AI: เนื้อหาบทความนี้ยังไม่ได้นำเข้าฐานความรู้ ให้ส่งลิงก์นี้ให้ครีเอเตอร์โดยตรง)_

---
