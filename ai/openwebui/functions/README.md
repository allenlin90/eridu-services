...
- **Uploaded filenames are flattened to their basename**, regardless of what path is sent in the multipart upload. A file uploaded as `shared/doc.md` is stored and returned as `doc.md`. Confirmed live: matching existing files by repo-relative path broke re-sync detection (the pipe saw no existing match, re-uploaded, and Open WebUI rejected the second copy with `400: Duplicate content detected`). Fixed by matching on the document's frontmatter `id` (already globally unique per `tools/validate-wiki`) instead of its path.
- **A first sync with `collection_id` unset in Valves creates a new collection on *every* trigger**, not just the first — the Pipe has no way to remember what it created last time. Capture the returned `collection_id` from the first run's report and set it in Valves immediately, or every subsequent trigger silently creates another "Company Wiki" collection.
