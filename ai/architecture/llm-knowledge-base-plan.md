...
- **Done.** Wiki structure, frontmatter schema, and validator are built and tested: `ai/openwebui/knowledge/company-wiki/{intake,content,generated,tools}`, `tools/wiki-schema.json` (sensitivity/status/audience vocabulary — Member/Team-Lead/Manager tiers per pillar, `org-general`/Admin auto-granted, never listed explicitly — Admins on every collection, `org-general` only on `public`/`internal` ones), `tools/validate-wiki` (required fields, enum, date, duplicate-id, and wikilink validation; `--write-manifest` emits `generated/wiki-manifest.json` with per-document `expanded_group_grants`). Validated against fixtures covering all five defect classes (missing/invalid fields, invalid enums, unknown audience tag, bad date ordering, duplicate id, broken wikilink) plus a clean pass with shorthand expansion (`company-wide` -> all 10 groups, pillar shorthand -> that pillar's 3 tiers).
