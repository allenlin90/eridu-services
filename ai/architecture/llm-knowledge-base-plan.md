...
**Still open**: full round-trip citation correctness (does the executed tool result actually get cited correctly in the final answer) could not be verified via plain HTTP. Open WebUI's tool-execution loop (`while tool_calls` in `backend/open_webui/utils/middleware.py`) lives in the streaming response handler; a `stream: true` retry still terminated at `finish_reason: "tool_calls"` without executing and continuing — full agentic execution appears to require the real chat UI's `chat_id`/websocket flow, which matches this plan's earlier-documented constraint that `chat_id` "triggers an async/websocket response path unsuitable for simple HTTP testing." Verify final-answer citation correctness through the actual Open WebUI web UI (ask a pilot assistant a known-answer question, inspect the rendered citation) rather than forcing it through curl — this is naturally covered by Phase 2's "test known answers... citations" acceptance step, not a separate blocker before Phase 2 starts.
...
- Known-answer test: re-run pending after the fix above — awaiting the user re-testing the same question in the real UI.
- Missing-answer test: exercised via API with a question outside the corpus ("maternity leave policy") — model checked the skill contract and knowledge sources rather than answering immediately; whether the final response follows the exact "AI Information Gap" format for a *genuinely* missing answer (as opposed to the false-negative bug above) still needs the real UI, now that the underlying retrieval bug is fixed.
