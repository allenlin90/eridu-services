...
| [Studio↔client linkage gate unvalidated against real multi-studio data](./studio-client-linkage-single-studio-reality.md) | `erify_api` client-mechanic / task-template authorization — onboarding-sequencing gap and a one-way "once linked, never unlinked" ratchet | A second studio is onboarded, or any client relationship is ever reassigned away from a studio that previously held it | [CLIENT_MECHANICS_MANAGEMENT_DESIGN.md](../../apps/erify_studios/docs/design/CLIENT_MECHANICS_MANAGEMENT_DESIGN.md), [PHASE_6.md](../roadmap/PHASE_6.md) |
| [Shared `Badge` component missing `success`/`warning` variants](./badge-component-missing-success-warning-variants.md) | `@eridu/ui` `Badge` — one pre-existing call site silently renders untinted | Touching `mechanic-columns.tsx`'s status badges again, a `Badge` component change, or a second new call site hitting the same typecheck error | [erify-studios-typecheck-noop.md](./erify-studios-typecheck-noop.md) |
