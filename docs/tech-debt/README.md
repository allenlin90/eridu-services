...
| [Resolve-dialog active-task guard renders a static message, not count+nav-link](./state-gate-resolve-dialog-active-task-static-message.md) | `erify_studios` show-cancellation-resolution panel — `ACTIVE_TASKS_REMAIN` mapped to a fixed string, no count/link | A studio reports the static message is insufficient, or the panel/hook is touched for an unrelated reason | [Show State Gate design](../superpowers/specs/2026-06-23-show-state-gate-design.md), PR #230 |
