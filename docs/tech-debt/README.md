...
| [Schedule publish's active-task check disagrees with the cancellation gate's](./schedule-publish-active-task-check-mismatch.md) | `erify_api` schedule publishing vs. cancellation gate — two different "active task" definitions for the same question | `publishing.service.ts` remove-path logic changes again, or a studio reports a show parked pending-resolution that the gate would have let resolve directly | [schedule-publish-gate-unification ideation](../ideation/schedule-publish-gate-unification.md) |
