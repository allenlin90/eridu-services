...
| [Schedule publish restore writes show status without an audit row](./schedule-publish-restore-no-audit.md) | `erify_api` schedule publishing — `toUpdate` restore loop bypasses the cancellation gate the same way the removal loop used to | `publishing.service.ts` diff/restore logic changes again, or cancellation/restoration reporting is added | [SHOW_CANCELLATION_GATE.md](../../apps/erify_api/docs/SHOW_CANCELLATION_GATE.md), PR #233 |
