...
- [ ] Generate a Prisma migration for `CompensationLineItem` and its enums using Prisma tooling.
- [ ] Implement the model repository/service with soft-delete, UID generation, Decimal string serialization, and target immutability.
- [ ] Implement `LineItemTargetResolver` with studio ownership checks for show, show creator assignment, shift, and shift block targets.
- [ ] Implement `/admin/compensation-line-items` CRUD/list using admin controller patterns and `@CurrentUser` actor resolution.
- [ ] Test every target type, missing/cross-studio target rejection, signed decimal round-trip, reason validation, update, and soft delete.
...
**Files:**
- Modify: `apps/erify_api/prisma/schema.prisma`
- Modify: `packages/api-types/src/shows/`
...
- [ ] Generate a Prisma migration adding `Show` actuals and `StudioShiftBlock` actuals.
- [ ] Add shared actuals input schemas with resource-specific names.
- [ ] Add show actuals and shift-block actuals endpoints with one-sided clear support and inverted-range validation.
- [ ] Implement `appendSnapshotAudit()` as a pure helper.
- [ ] Wire future snapshot edits to append audit entries for changed `ShowCreator` agreement fields and `StudioShift.hourlyRate`.
...
- [ ] Test set/clear/inverted actuals, snapshot audit append/no-append, override reason, and unresolved new-write metadata.
- [ ] Verify backend lint, typecheck, tests, and build.
...
- [ ] Add show actuals input on the show workflow surface.
- [ ] Add snapshot warning dialog to assignment compensation edits.
...
- [ ] Remove backend create/update/persistence logic and response serialization for the dropped fields.
