...
Resolution: a manager opens the gate from `task-review` (filtered to `Task Type = State Gate`, unassigned) or from the show detail page, and either:
...
4a. Create `.agent/skills/schedule-publish-removal-resolution/SKILL.md` — same shape, for the second gate kind: why it's unassigned by default, the two outcomes (`CANCELLED` vs `RESTORE_PREVIOUS`) and what each means, the `schedule_resume_notice` planner-notice mechanism and when it clears, and a "read this before changing" list covering `publishing.service.ts`'s remove-flow and the schedule-continuity docs.
