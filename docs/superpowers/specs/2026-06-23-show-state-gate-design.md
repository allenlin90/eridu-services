...
PR #229's migration (`20260623120000_show_cancellation_resolution`) and the `ShowCancellationResolution`/`ShowCancellationResolutionOwner` relations it added to `User`/`StudioMembership`/`Show` are reverted as part of implementing this design, replaced by the single `TaskType.STATE_GATE` enum addition. PR #229 was never merged to `master`, so there is no production `ShowCancellationResolution` data to backfill or migrate — this is a pure schema/code revert, not a data migration.
