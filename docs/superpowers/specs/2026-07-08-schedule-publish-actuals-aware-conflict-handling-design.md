...
- New resolve-service tests: apply happy path, apply-after-drift conflict (`CONFLICT_STATE_CHANGED`), dismiss, double-resolve race (both requests fired concurrently against the same `conflict_uid`, second gets `CONFLICT_ALREADY_RESOLVED` — needs an actual concurrency test, not just sequential calls, to exercise the advisory lock), `removal_held_back` apply re-evaluating the active-task count live rather than trusting the snapshot.
