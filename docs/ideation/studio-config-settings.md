...
*   **Open questions for the eventual design pass:** where enforcement actually blocks (task submission? show `COMPLETED` transition? both?), whether requirements are per-show-standard or per-client (mechanics are client-owned, not studio-owned — see [`CLIENT_MECHANICS_MANAGEMENT_DESIGN.md`](../../apps/erify_studios/docs/design/CLIENT_MECHANICS_MANAGEMENT_DESIGN.md)), and how an account manager's sign-off/requirement request (today an out-of-band conversation) should flow into this config instead of staying tribal knowledge.
