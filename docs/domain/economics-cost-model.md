...
- `ShowPlatform` performance facts — nullable typed columns for GMV, viewer count, CTR, and CTO, consumed by [`show-performance-analytics.md`](../features/show-performance-analytics.md).
...
| `ShowPlatform` | One platform stream's actual window plus platform performance facts such as GMV, views, CTR, and CTO. | ✅ In scope |
...
Each operational time or attendance fact carries an independently selected source category. The cascade below describes the **per-fact resolver**: when a new input event arrives, the resolver compares the incoming source against the previously selected source for that specific fact and writes only if the incoming priority is higher or equal. Platform performance facts use template-based provenance on `ShowPlatform.metadata.performance_templates`, because their precedence is based on the task template that supplied the metric. The product contract needs selected-source/provenance context per fact; ingestion details and lower-priority reference inputs stay behind the service layer unless a review surface needs to show them.
...
Source values use the short forms defined by `actualsSourceSchema` in [`packages/api-types/src/audits/schemas.ts`](../../packages/api-types/src/audits/schemas.ts): `MANAGER`, `PLATFORM`, `CREATOR_INPUT` (reserved), `OPERATOR`, `PLANNED`. Time-pair facts use `metadata.actuals_source`; platform performance facts use `metadata.performance_templates` keyed by `system_fact_key` so a single `ShowPlatform` row can mix a manager-overridden time pair with task-sourced GMV or view counts. The row's `actuals_source` exposed by the calculator projects only the time-pair source for backward compatibility with PR 4 / 10 / 11 self-views; non-time facts are not currently surfaced as their own row-level fields.
