...
- `ShowPlatform` performance facts — **deferred to the PR 21 analytics infrastructure investigation** (see [`show-performance-analytics-infra.md`](../ideation/show-performance-analytics-infra.md)). Phase 4 does not promote GMV, view count, CTR/CTO, or any other platform performance metric to a typed `ShowPlatform` column. `viewerCount` keeps its pre-existing `Int @default(0)` shape from the init migration but its read path is analytical.
