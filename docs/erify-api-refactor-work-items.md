...
- **Progress:**
  ...
  - **(b2)** studio-performance metrics/calculator service — extract the pure metric/loop logic (`mapShowToPerformance`, sort helpers, `buildLoopItems`, `sumShowStoredAggregates`, `computePeakFromLoops`, `maxDecimal`) into an `@Injectable() StudioPerformanceCalculatorService` to clear <600 (707 today). Pending.
  - **(c)** remove `Record<string,any>` metadata casts once WI-11 schemas land — pending.
