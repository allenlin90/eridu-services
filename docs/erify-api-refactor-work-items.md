...
| WI-T-analytics (shift cost) | 🔄 in review | #174 | Phase 2 characterization (unblocks WI-21): direct pins for the public `calculateShiftCost` sibling (per-block actual-vs-planned duration + warning, `hourlyRate*hours` base, block+shift line items, dominant `actuals_source` precedence). +3 tests. |

**Baseline now:** 134 suites / 1219 tests (at #173; #174 keeps it green).
