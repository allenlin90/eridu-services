...
- **Mechanic→shows coverage**: per mechanic, which templates reference it (and whether the latest version still does) and, per target show, whether content is **current / stale / dropped / unassigned** (20.6).
...
- [x] Mechanic→shows coverage resolves per-show status to current/stale/dropped/unassigned using frozen vs. current `contentRevision` and template-latest membership; problem rows offer read-only Flag to Manager.
