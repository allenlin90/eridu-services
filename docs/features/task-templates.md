...
| `system_fact_key`              | System-fact picker; selecting a fact sets the compatible field type from `@eridu/api-types/task-management` | No runtime effect until PR 12 hydration; later expands into target-scoped inputs | Later PR 12 review/extraction surfaces read the bound fact |
