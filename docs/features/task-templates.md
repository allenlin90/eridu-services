...
### Layer 2 — Snapshot (mostly immutable, append-only for hydrated bindings)

`TaskTemplateSnapshot.schema` is a byte-for-byte copy of `currentSchema` at task-creation time. `Task.snapshotId` pins each task to the exact shape it was created with. Editing the template later **never** mutates past snapshots. **Non-hydrated, manually-authored template fields are immutable for the life of the snapshot.**

**Exception 1 — `system_fact_key` hydrated bindings (PR 12 series).** Fields generated by the task-input fact binding engine for per-creator or per-platform targets (e.g., `fld_attendance_missing_creator_<creatorUid>`, `fld_gmv_platform_<platformUid>`) are **append-only mutable** while a task is not yet submitted: re-rendering the task reconciles its hydrated fields against the current `ShowCreator` / `ShowPlatform` set, appending new fields for newly assigned targets and marking fields whose targets were unassigned afterwards as `binding_stale: true`. Already-hydrated fields whose target is still assigned are untouched (operator's in-progress values preserved). Once a task is submitted, the snapshot freezes — no rehydration occurs after submission. See [TASK_INPUT_FACT_BINDING_DESIGN.md](../../apps/erify_api/docs/design/TASK_INPUT_FACT_BINDING_DESIGN.md) §3.A.
...
| `system_fact_key`              | "Save answer as" picker; selecting a binding sets the compatible field type from `@eridu/api-types/task-management` | No runtime effect until PR 12 hydration; later expands into target-scoped inputs | Later PR 12 review/extraction surfaces read the bound fact |
