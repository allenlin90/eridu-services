...
| `system_fact_key`  | —                   | closed operational fact binding       | frozen on authored fields; per-target hydrated keys live in `task.content` and are recomputed at render and submission time until the task is `COMPLETED`/`CLOSED` | drives target-scoped hydration in PR 12 | read by PR 12 extractors; platform performance bindings are also rolled up by task reports |
