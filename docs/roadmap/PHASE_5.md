...
| 9   | [Show-level issue ownership](#9-show-level-issue-ownership) — narrow issue record for show blockers and extraction-detected anomalies without state-gate enforcement                                       | —                                         | ✅ Done |
...
| 11  | [Advisory planning readiness checklist](#11-advisory-planning-readiness-checklist) — aggregate current planning readiness signals without enforcing a status transition                                    | 1, 2                                      | 🚧 In progress |
...
**Completion result**: PR [#335](https://github.com/allenlin90/eridu-services/pull/335)
makes admin and studio generic show edits share a server-side policy that
rejects entering or leaving `CANCELLED` and
...
**Source**: [`show-production-lifecycle`](../../.agents/skills/show-production-lifecycle/SKILL.md) skill — Lifecycle Phases §2; [`Show-Level Issue Ownership`](../../apps/erify_api/docs/SHOW_ISSUE_OWNERSHIP.md); [`late-material-edit-audit-policy.md`](../ideation/late-material-edit-audit-policy.md)

A dedicated `ShowIssue` operational record for creator attendance, equipment dysfunction, utility outage, platform violations, and post-production follow-up. Each issue has an owner, due date, severity, evidence, status, manual escalation state, optimistic version, and resolution record. Issues are advisory records for review and later export; they do not drive state transitions until items 18 and 19.
...
The API uses one studio-scoped `/show-issues` collection with UID-only external identifiers and real database pagination. Show detail has an Issues tab, and Show Run Review has a lean unresolved count plus a lazy paginated Issues tab. Assignment, severity, escalation, resolution, reopening, and automated evidence changes use standard `Audit` history.

**Delivery program**: shipped as a fullstack feature (backend API + frontend Issues tab) through [`integration-pr-delivery`](../../.agents/workflows/integration-pr-delivery.md) rather than direct-to-`master` PRs, since landing the backend alone would have left a complete API with no operator-visible workflow. Main integration PR: [#357](https://github.com/allenlin90/eridu-services/pull/357). Breakdown children, all merged: model/migration/manual workflow API ([#356](https://github.com/allenlin90/eridu-services/pull/356)), show-detail Issues tab ([#358](https://github.com/allenlin90/eridu-services/pull/358)), automated attendance/platform-violation reconciliation ([#361](https://github.com/allenlin90/eridu-services/pull/361)), Show Run Review summary counts and paginated Issues tab ([#365](https://github.com/allenlin90/eridu-services/pull/365)).

**Architecture boundary**: explicit `ShowIssueWorkflowService` and `ShowIssueReconciliationService` orchestration, not a generic event bus or NestJS CQRS. The [notification PRD](../prd/notification-system.md) owns the durable event/outbox capability; item 15 activates the issue publisher after that foundation exists.
...
**In-progress brief (August 2026)**: shared contract shipped in `@eridu/api-types/shows` (`LIFECYCLE_READINESS_PHASE`, `LIFECYCLE_CONDITION_STATUS`, `lifecycleConditionSchema`, `showPlanningReadinessSchema`) — item 12 reuses the same enums/condition shape under its own `completion_review` phase literal. Backend: new `PlanningReadinessService` (show-orchestration capability) computes all five conditions (room, creators, platforms, task stages generated, tasks assigned) per show from an explicit show-id list, not a re-derived date window, so it stays decoupled from `ShiftAlignmentService`'s planning-risk scoping; the task-stage/assignment logic that used to live only in `ShiftAlignmentService` moved to a shared `show-task-coverage.util.ts` both now import (behavior-preserving — existing shift-alignment spec still green). Two ADMIN/MANAGER-gated routes: `GET /studios/:studioId/shows/:id/planning-readiness` (single) and `GET /studios/:studioId/shows/planning-readiness?show_id=...` (bulk-by-ids, capped at 100). Frontend: a Planning Readiness card on show detail, and a per-row readiness column on the `/task-setup` shows table (bulk query scoped to the current page's show ids) — `ShowReadinessTriagePanel` itself is untouched, per the note above. Verification green (`erify_api` lint/typecheck/test/build, `erify_studios` lint/typecheck/test/build); not yet committed/PR'd.
...
**Source**: [`show-production-lifecycle`](../../.agents/skills/show-production-lifecycle/SKILL.md) skill — Lifecycle State Machine; [`Show Cancellation Gate`](../../apps/erify_api/docs/SHOW_CANCELLATION_GATE.md); [`erify_api` Architecture](../../apps/erify_api/docs/ARCHITECTURE.md#trigger-gated-next-steps); ideation docs [`schedule-publish-gate-unification.md`](../ideation/schedule-publish-gate-unification.md) and [`cancellation-gate-note-amendment.md`](../ideation/cancellation-gate-note-amendment.md)

**Architecture activation**: item 18 activates the trigger-gated `ShowOperationsModule` direction in the current architecture. Implement the transition service inside that capability; do not create a fifth parallel writer or perform a standalone folder move. Its schedule-publish integration activates the `PublishingService` decomposition gate only when that integration requires it, or when measured query, lock, rollback, or maintainability risk independently reaches the gate.
