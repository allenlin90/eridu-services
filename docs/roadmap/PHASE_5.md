...
| 9   | [Show-level issue ownership](#9-show-level-issue-ownership) — narrow issue record for show blockers and extraction-detected anomalies without state-gate enforcement                                       | —                                         | 🚧 In progress |
...
**Delivery program (2026-08-02)**: this is a fullstack feature (backend API + frontend Issues tab), so it ships through [`integration-pr-delivery`](../../.agents/workflows/integration-pr-delivery.md) rather than direct-to-`master` PRs — landing the backend alone would leave a complete API with no operator-visible workflow. One main integration PR ([#357](https://github.com/allenlin90/eridu-services/pull/357)) tracks the breakdown checklist and owns the single merge to `master`; child PRs target the integration branch. Backend (Delivery Sequence steps 1–2: model, migration, manual workflow API) merged into the integration branch as [#356](https://github.com/allenlin90/eridu-services/pull/356); frontend (step 3: show-detail Issues tab) is in review as a second child PR ([#358](https://github.com/allenlin90/eridu-services/pull/358)) branched from the backend work. Steps 4 (automated reconciliation) and 5 (Show Run Review) are separate follow-up children once 1–3 land.
