...
Add show-time/change-time/impact-kind/status filters and accurate server-side summary counts to `/studios/:studioId/schedule-publish-impacts`. Persist a `PublishRun` model so a Google Sheets (or future studio-native) publish is a first-class, filterable audit unit instead of an implicit `Audit.createdAt` cluster. Add a narrowly-scoped, fill-gap-only creator-mapping backfill for past shows with no `ShowCreator` mapping, as its own explicitly manager-triggered action (own method, endpoint, and `PublishRun` source) rather than default behavior of the routine publish flow — keeps the load-bearing `publish()` path untouched until the new action has proven out. Existing mappings are never overridden, since `ShowCreator` can be targeted by a `CompensationLineItem` and no settlement/freeze guard exists yet (see spec §3, §5).
