...
| #   | Workstream                                                                                                                                                                                                 | Depends on                                | Status         |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------- | -------------- |
| 7   | [Schedule publish impact review enhancements](#7-schedule-publish-impact-review-enhancements) — filters, persisted publish-run batches, and scoped past-show creator-mapping backfill for `/schedule-publish-impacts`; orthogonal to the state-machine critical path | — (informational: 4, 9, 11/19 — see brief) | 🚧 In progress |
...
Add show-time/change-time/impact-kind/status filters and accurate server-side summary counts to `/studios/:studioId/schedule-publish-impacts`. Persist a `PublishRun` model so a Google Sheets (or future studio-native) publish is a first-class, filterable audit unit instead of an implicit `Audit.createdAt` cluster. Add a narrowly-scoped, fill-gap-only creator-mapping backfill for past shows with no `ShowCreator` mapping: the Sheet already sends per-show `creators` data, so backfill stays on the same `/validate`/`/publish` API and reuses that data via a bounded, separately-tested service step rather than a new endpoint or button — `publish()`'s main diff+upsert path is untouched, and `/validate` surfaces the backfill-eligible count non-blockingly before commit. Existing mappings are never overridden, since `ShowCreator` can be targeted by a `CompensationLineItem` and no settlement/freeze guard exists yet. Relationship to items 4, 9, 11/19 documented in the spec (see spec §3, §5, "Relationship to Other Phase 5 Items") — informational, not blocking.

**Priority note (July 2026 review)**: no Phase 5 item depends on item 7; it is orthogonal to the state-machine critical path and can proceed in parallel, but it must not displace item 10's design work, which actually gates item 12.
