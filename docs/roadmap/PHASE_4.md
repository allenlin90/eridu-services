...
- ✅ **PR 12.0.4 — Per-target field hydration in operator task forms** ([#95](https://github.com/allenlin90/eridu-services/pull/95)): pure `hydrateTaskFormSchema` in `@eridu/api-types/task-management`; task read responses carry `show_creators` / `show_platforms` UIDs and a `hydration_context` block; bound fields expand to one input per assigned `ShowCreator` / `ShowPlatform` keyed by `<fieldId>__<scope>__<targetUid>`; stale hydrated keys render dimmed and read-only with `require_reason` stripped so a removed target can't block submission. No DB migration: the template snapshot stays immutable; hydration runs at render and validation time. Extractor wiring lands in 12.0.5.
