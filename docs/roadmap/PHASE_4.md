...
> **Last updated**: 2026-05-24 · **Status**: 🚧 Active · **Remaining**: 17 PRs · **Next**: 12.3.2 ShowPlatformViolation extractor
...
- **Analytics investigation**: [`show-performance-analytics-infra.md`](../ideation/show-performance-analytics-infra.md) — post-12.4 OLAP/read-model topic
...
- ✅ **PR 12 design locked** ([#90](https://github.com/allenlin90/eridu-services/pull/90)): [`TASK_INPUT_FACT_BINDING_DESIGN.md`](../../apps/erify_api/docs/design/TASK_INPUT_FACT_BINDING_DESIGN.md). Operational implementation is split into 12.0.1-12.4 below; analytics infrastructure is tracked as a post-12.4 investigation.
...
| 12.0.2 | [Phase 4 actuals schema additions](../prd/task-fact-binding.md#section-a-foundation-prs-1201--1205) — single Prisma migration materializing design doc §2.A: nullable actuals time columns + indexes on `Show` / `ShowCreator` / `ShowPlatform`; `attendanceMissing` / `attendanceReason` on `ShowCreator`; empty `ShowPlatformViolation` table. **No** `gmv` / `performanceMetrics` / new `viewerCount` index — platform performance metrics are analytical and deferred to 12.5. No consumers wired. | —                    | ✅ Shipped                    | [#92](https://github.com/allenlin90/eridu-services/pull/92)                |
...
| 12.2.1 | [Split `ShowCreator.attendanceReason` column](../../apps/erify_api/docs/design/TASK_INPUT_FACT_BINDING_DESIGN.md) — replace the shared column with `lateAttendanceReason` + `missingAttendanceReason` to retire the runtime ownership-inference scheme PR 12.2 had to land (eight Codex iterations, all symptoms of the column overload). Schema migration + extractor refactor + read-model derivation update; see "Anti-patterns from PR 12.2 (column overloading)" in `fact-extraction-pipeline` skill for context. | PR 12.2              | 🔲 Planned                    | —                                                                         |
| 12.3.2 | [`ShowPlatformViolation` extractor](../prd/task-fact-binding.md#section-b-extractors-prs-1211--1232) — replace-all scoped to `(sourceTaskId, sourceFieldId)`. Table already exists from 12.0.2.                                                                                                   | PR 12.1.2            | 🔲 Planned                    | —                                                                         |
...
| 12.5   | [Show performance analytics infrastructure investigation](../ideation/show-performance-analytics-infra.md) — post-12.4 design decision on whether to extend current services with read models/materialized views or introduce OLAP infrastructure for show-level analytical features. Covers GMV, viewer count, CTR/CTO/likes/concurrents, and cross-show aggregates that 12.0.2 deliberately deferred. | PR 12.4              | 🔲 Planned investigation      | —                                                                         |
| 13     | [Economics review surface](#pr-13--economics-review-surface) (`/studios/:id/finance/economics`) — cost-reference read model. GMV/views are analytical and deferred to 12.5; this row doesn't block on them. Revenue / contribution margin / commission deferred to Phase 5. | PR 12.1.2, PR 12.2   | 🔲 Planned                    | —                                                                         |
