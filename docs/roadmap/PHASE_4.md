...
> **Last updated**: 2026-05-30 · **Status**: 🚧 Active · **Remaining**: 13 PRs · **Next**: 12.6 Show performance analytics infrastructure investigation
...
- ✅ **PR 12.4.7 — Show Run Review CSV export**: each `/show-run-review` tab (creators / violations / tasks / shows) gets an Export CSV action that refetches the tab's full filtered set (`limit = total`, same active `search`/`status`/`severity`/`completeness` filters) and serializes **every matching row — the full filtered result, not the current page**. Client-side serialization via the shared `@/lib/csv` + `@/lib/file-download` utils; no new endpoint. Per-tab pending/disabled state; no-op when the filtered total is 0.
...
| 12.4.7 | [Show Run Review CSV export](../prd/task-fact-binding.md#section-c-review-surface-pr-124) — export the full filtered set of each run-review tab (creators / violations / tasks / shows) to CSV. | PR 12.4.4            | ✅ Shipped                    | —                                                                         |
