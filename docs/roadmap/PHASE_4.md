...
| 12.1.2 | [ShowPlatform actuals extractor + per-target paired writes + stale-target guard](../prd/task-fact-binding.md#section-b-extractors-prs-1211--1232) — `ShowPlatformActual{Start,End}TimeExtractor` registered against the 12.0.2 columns; per-target atomic paired write in `applyPairedShowPlatformActuals` (one platform's pair commits/rolls back independently of others); bulk-resolved active-target cache + `skipped_stale_target` outcome for un-assigned / soft-deleted platforms; same priority resolver + cross-task collision routing as 12.1.1. | PR 12.1.1            | ✅ Shipped                    | TBD                                                                       |
