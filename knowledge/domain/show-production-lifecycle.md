---
type: domain_concept
title: Show Production Lifecycle
description: Livestream Show state machine, entity graph, lifecycle phases, readiness conditions, and operating roles for eridu-services.
status: stable
stale_after: "2027-01-01"
sources:
  - title: Show and ShowStatus schema
    path: apps/erify_api/prisma/schema.prisma
  - title: Show issue ownership contract
    path: apps/erify_api/docs/SHOW_ISSUE_OWNERSHIP.md
  - title: Phase 5 roadmap
    path: docs/roadmap/PHASE_5.md
---

# Show Production Lifecycle

The show is the central operational record in eridu-services. Every schedule, creator assignment, task, shift, platform stream, performance metric, and compensation line item connects through or references a show. This concept defines the lifecycle contract, entity relationships, and operational boundaries that agents must understand before changing show-related code. Selected by the [`show-production-lifecycle`](../../.agents/skills/show-production-lifecycle/SKILL.md) skill.
...
For field-level detail on each entity, see [references/entity-relationships.md](../../.agents/skills/show-production-lifecycle/references/entity-relationships.md).
...
**Fact extraction**: When tasks are approved, the fact-extraction pipeline writes actuals to Show/ShowCreator/ShowPlatform entities. Source priority: MANAGER (4) > PLATFORM (3) > CREATOR_INPUT (2) > OPERATOR (1) > PLANNED (0). See the `fact-extraction-pipeline` skill.
...
For the full condition inventory and enforcement-level design, see [references/state-gates.md](../../.agents/skills/show-production-lifecycle/references/state-gates.md).
...
| Layer | Key files |
| --- | --- |
| Schema | `apps/erify_api/prisma/schema.prisma` — Show, ShowCreator, ShowPlatform, ShowStatus |
