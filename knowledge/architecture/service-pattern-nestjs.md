...
title: NestJS Service Pattern
description: Canonical service-layer doctrine for erify_api — capability and model boundaries, ORM decoupling, and error handling by service type.
status: stable
stale_after: "2027-01-01"
sources:
  - title: erify_api architecture and persistence matrix
    path: apps/erify_api/docs/ARCHITECTURE.md
  - title: Canonical model service
    path: apps/erify_api/src/models/task-template/task-template.service.ts
  - title: Base model service
    path: apps/erify_api/src/lib/services/base-model.service.ts
---
...
> provider. This concept defines the service correctness rules for either choice.
...
- **Money / `Prisma.Decimal`:** `Decimal` is a `Prisma.*` type — don't expose it in public service signatures; convert `Decimal` → string at the boundary. Format with the canonical `decimalToString` (`lib/utils`); don't add parallel money formatters with divergent semantics. A shared domain `Money` type is a deferred direction decision (see [`docs/tech-debt/erify-api-refactor-residuals.md`](../../docs/tech-debt/erify-api-refactor-residuals.md)).
