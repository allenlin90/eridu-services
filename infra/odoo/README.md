...
- `build.watchPatterns` in `.railway/odoo.json` is scoped to `/infra/odoo/**`, matching every other `.railway/<service>.json` in the repo. Without it, Railway falls back to watching the whole repo and redeploys Odoo on every push to `master`, including changes with no relation to this service.
