...
All of this was verified locally end-to-end (build → throwaway Postgres → preDeploy command run twice to confirm idempotency → start command) before shipping: `auth_oidc` installs cleanly and fires the hook exactly once, the manager correctly shows "disabled by the administrator", the master password from the env var is what's actually in `odoo.conf`, and the rotated admin password is what actually logs in over HTTP.
