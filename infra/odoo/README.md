...
- **`patches/auth_oidc/eridu_bootstrap.py`**, wired as `auth_oidc`'s `post_init_hook` in `__manifest__.py`. Odoo's non-interactive `-i` bootstrap creates the `admin` login with the literal default password `"admin"`; this rotates it to a random strong value. Naturally one-shot: `post_init_hook` only fires when a module is genuinely installed for the first time, never on a later `-i` of an already-installed module — so it can never clobber a password someone set through the UI. Belt-and-suspenders on top of that: it also verifies the current password hash against `"admin"` first (the same way Odoo's own `_check_credentials` does — `res.users.password` is never readable via a plain ORM read, so this reads the hash with raw SQL) and no-ops if it's already been changed.
...
`deploy.preDeployCommand` in `.railway/odoo.json` runs `-i auth_oidc` on every deploy (idempotent — Odoo's `-i` no-ops once a module is installed), which transitively installs `base`/`auth_oauth` too and fires the hook above on the first run. This means the entire database — schema, `auth_oidc`, a real admin login — is created automatically on first deploy; there's no manual Apps-screen step and no need to ever re-enable the (deliberately hardened) `/web/database/manager` UI.

All of this was verified locally end-to-end (build → throwaway Postgres → preDeploy command run twice to confirm idempotency → start command → retrieve via `ir.config_parameter` → actual HTTP login with the retrieved password) before shipping.
