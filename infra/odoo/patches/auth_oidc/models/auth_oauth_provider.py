...
#
# Patched for eridu-services (see ../../../README.md): upstream OCA/server-auth
# auth_oidc verifies id_tokens with python-jose, which has no EdDSA support.
# eridu_auth signs id_tokens with EdDSA/Ed25519, so _decode_id_token below is
# rewritten against PyJWT[crypto] (which does support EdDSA) instead of
# python-jose. Everything else in this file is unchanged from upstream — diff
# against the original at models/auth_oauth_provider.py in the pinned commit
# when bumping OCA_SERVER_AUTH_COMMIT to see what else may need re-applying.
#
# Known gap vs. upstream: python-jose's decode() took an `access_token` kwarg to
# validate the `at_hash` claim (binds the access_token to the id_token). PyJWT
# has no equivalent option, so at_hash is not checked here. Low risk for this
# flow specifically (server-to-server token exchange with a client secret, not
# a public/implicit client), but a real gap if this provider record is ever
# reused for a public-client flow.
...
try:
    ...
    logging.getLogger(__name__).debug("PyJWT library not installed")
