...
- **`StudioShift.hourly_rate`**: inline on the **shift compensation dialog**'s `Hourly rate` tile (PR 3.5). The tile flips to a money input + required `override_reason` textarea + Save / Cancel. Save with the unchanged rate just closes the editor without firing a PATCH; a changed rate without a reason keeps Save disabled. The edit-shift dialog no longer carries an `hourly_rate` field.
