...
- `held_back.show_creators[]`/`show_platforms[]` render the resolved `creator_name`/`platform_name` the payload carries, falling back to the uid when it is absent (older conflicts recorded before the name was resolved, or an entity that no longer resolves); a uid is safe to display per this app's external-ID strategy, just not human-readable
