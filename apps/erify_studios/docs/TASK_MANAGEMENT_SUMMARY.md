...
- **Loop content**: Flat `task.content` JSON covering all loops, including sidecars such as `<fieldKey>__reason`; v1 content is keyed by `field.key`, v2 content by `field.id`; `JsonForm` is filtered by `activeGroup` (loop ID) in execution sheet only
