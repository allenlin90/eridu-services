...
### 7.1 Scene QC Evidence Designation (Admin/Manager)
Task Templates → Create/Edit Template → open an image-only file field → enable **Use as Scene QC evidence**. The field editor persistently explains that uploads also appear in the separate Scene Review workspace and that Manager Review approval is not required. Enabled fields receive highlighted treatment plus a `Scene QC` field-card badge. Saving the template creates the snapshot-level evidence reference; later Task submission only stores the uploaded image value.
...
Scene QC → pick an operational day → review each eligible Show's explicit image evidence against the Client's Scene Profile reference → record `PASS`, `MINOR`, or `FAIL` with feedback required for Minor and Fail → confirm the completed day → open the manager report or download its CSV. A Records tab queries confirmed history by date range, Client, platform, and result. Client uses the shared asynchronous combobox; platform and result are secondary filters. Desktop pairs the Show queue with a side-by-side evidence/reference workspace; mobile uses a Live/Expected drawer with the result form immediately below. Scene QC persists only its own outcomes and confirmations — it never changes Task, Manager Review, or Show state, and its snapshot bindings are created by Task Template saves rather than Task submissions. See [Scene QC](./SCENE_QC.md).
