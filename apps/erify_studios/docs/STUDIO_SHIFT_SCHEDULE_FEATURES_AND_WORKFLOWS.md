...
- Frontend admin planning cards now consume these endpoints on dedicated surfaces:
  - `/studios/:studioId/task-setup` for date-range task-readiness summary warnings.
  - `/studios/:studioId/shifts` for shift cost snapshot.
- `/studios/:studioId/task-setup` Show Readiness panel supports date-range querying and uses an admin triage layout:
...
- Added frontend API hooks for `shift-calendar`/`shift-alignment` and admin planning cards on dedicated surfaces: `/studios/:studioId/task-setup` (risk warning summary) and `/studios/:studioId/shifts` (cost snapshot).
...
1. **Show alignment orchestration** — baseline planning warning summary is implemented in `/studios/:studioId/task-setup`; pending dedicated report/drill-down views.
