...
4. **Specialized access stays narrow**: DESIGNER adds the Scene QC workspace — persisted scene outcomes, daily confirmation, and Client Scene Profiles — to member-level access, without Task Review or any task mutation; MODERATION_MANAGER adds its separately gated reporting tools.
...
| `/studios/:studioId/scene-review` | View/Review | View/Review | No access | View/Review | No access | No access |
...
| Tasks (role-filtered entries) | ADMIN, MANAGER; DESIGNER sees Scene QC only (labelled "Scene Review"); MODERATION_MANAGER sees Task Reports only |
