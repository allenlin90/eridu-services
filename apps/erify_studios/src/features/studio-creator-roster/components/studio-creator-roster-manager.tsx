...
import {
  type StudioCreatorRosterItem,
  useCreateStudioCreatorRosterItem,
  useDeleteStudioCreatorRosterItem,
  useStudioCreatorCatalog,
  useStudioCreatorRoster,
  useUpdateStudioCreatorRosterItem,
} from '../api/studio-creator-roster';

type StudioCreatorRosterManagerProps = {
  studioId: string;
};
...
function toEditableDefaults(item: StudioCreatorRosterItem): EditableRosterFields {
  return {
...
export function StudioCreatorRosterManager({ studioId }: StudioCreatorRosterManagerProps) {
  const [isAddDialogOpen, setIsAddDialogOpen] = useState(false);
  const [catalogSearch, setCatalogSearch] = useState('');
  const [selectedCatalogCreatorId, setSelectedCatalogCreatorId] = useState('');
  const [editingItem, setEditingItem] = useState<StudioCreatorRosterItem | null>(null);
  ...
  const rosterQuery = useStudioCreatorRoster(studioId, {
  ...
  const catalogQuery = useStudioCreatorCatalog(studioId, catalogSearch);
  const createRosterItem = useCreateStudioCreatorRosterItem(studioId);
  const updateRosterItem = useUpdateStudioCreatorRosterItem(studioId);
  const deleteRosterItem = useDeleteStudioCreatorRosterItem(studioId);
  ...
  const selectedCatalogOption = useMemo(
    () => catalogOptions.find((option) => option.value === selectedCatalogCreatorId),
    [catalogOptions, selectedCatalogCreatorId],
  );
  ...
  const openEditDialog = (item: StudioCreatorRosterItem) => {
    setEditingItem(item);
    ...
    <div className="space-y-4">
      ...
      >
        <DialogContent>
          ...
          <div className="space-y-3 py-2">
            <AsyncCombobox
              value={selectedCatalogCreatorId}
              onChange={setSelectedCatalogCreatorId}
            ...
            {selectedCatalogOption && (
              <div className="flex items-center justify-between rounded-md border bg-muted/30 px-3 py-2 text-sm">
                ...
                <Button
                  ...
                  variant="ghost"
                  size="sm"
                  onClick={() => setSelectedCatalogCreatorId('')}
                  disabled={createRosterItem.isPending}
                >
                  Clear
                </Button>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={!selectedCatalogCreatorId || createRosterItem.isPending}
            >
