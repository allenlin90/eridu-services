...
import { useShowTypes } from '../use-show-types';
...
vi.mock('@/features/show-types/api/create-show-type', () => ({
  useCreateShowType: vi.fn(() => ({
    mutateAsync: vi.fn(),
    isPending: false,
  })),
}));

vi.mock('@/features/show-types/api/update-show-type', () => ({
  useUpdateShowType: vi.fn(() => ({
    mutateAsync: vi.fn(),
    isPending: false,
  })),
}));

vi.mock('@/features/show-types/api/delete-show-type', () => ({
  useDeleteShowType: vi.fn(() => ({
    mutateAsync: vi.fn(),
    isPending: false,
  })),
}));

describe('useShowTypes', () => {
  const createWrapper = () => {
  ...
  it('should return show types data and mutations', () => {
    const { result } = renderHook(() => useShowTypes(), {
