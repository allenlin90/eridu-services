/**
 * Shared HTTP request utility for backdoor test scripts
 *
 * Automatically includes X-API-Key header if BACKDOOR_API_KEY
...
/**
 * Make HTTP request with automatic backdoor API key header inclusion
 *
 ...
 */
export async function backdoorHttpRequest<T = unknown>(
  method: string,
  url: string,
  body?: unknown,
  ...
  // Include API key header if BACKDOOR_API_KEY is configured in .env file
  // This matches the server's configuration and allows testing actual behavior:
  // - If key is in .env: header is sent → auth is enforced (dev or prod)
  // - If key is NOT in .env: no header sent → can test bypass behavior
  const apiKey = process.env.BACKDOOR_API_KEY;
  if (apiKey) {
