...
export async function httpRequest<T = unknown>(
  ...
  if (apiKey) {
    ...
    const preview = apiKey.length > 10 
      ...
      : '***';
    console.log(`🔑 Sending API key header (length: ${apiKey.length}, preview: ${preview})`);
  } else {
    console.error('❌ GOOGLE_SHEETS_API_KEY not found in environment variables');
    console.error('   Available env vars:', Object.keys(process.env).filter(k => k.includes('API') || k.includes('KEY')).join(', ') || 'none');
    console.error('   This request will fail if the endpoint requires authentication');
  }
