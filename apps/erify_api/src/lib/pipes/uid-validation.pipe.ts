...
export class UidValidationPipe implements PipeTransform<string, string> {
  constructor(
    private readonly expectedPrefix: string | string[],
  ...
  transform(value: string): string {
    ...
    const prefixes = Array.isArray(this.expectedPrefix)
      ? this.expectedPrefix
      : [this.expectedPrefix];
    ...
    const matchedPrefix = normalizedPrefixes.find((prefix) => value.startsWith(prefix));
    if (!matchedPrefix) {
      throw new BadRequestException({
        statusCode: 400,
