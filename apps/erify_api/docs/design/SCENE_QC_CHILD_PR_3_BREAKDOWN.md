...
| # | Decision |
| --- | --- |
| OQ-1 | **Superseded during PR #348 review (2026-07-28).** `SceneQcReviewEvidence.objectKey` stays a nullable column (defensive/legacy), but `SceneQcEvidenceResolver` now EXCLUDES a designated value from the resolved set when `deriveObjectKeyFromPublicUrl` returns `null`, instead of pinning it with `objectKey: null`. Rationale for the reversal: the only legitimate producer of a Scene QC evidence value is the presign-backed file upload widget, which always writes a public R2 URL — a non-derivable URL means foreign/untrusted content, not a legitimate gap, and rendering it unverified in the reviewer's browser was the real risk the original recommendation underweighted. A Show whose only evidence is non-R2 becomes `NO_EVIDENCE`-blocked rather than reviewable against unverified content. |
...
*Recommendation (superseded)*: originally, make `objectKey` nullable on `SceneQcReviewEvidence`, treat `fileUrl` as the render source, and add `deriveObjectKeyFromPublicUrl` as best-effort provenance, tolerating a `null` result. *Final decision, made explicitly during PR #348 review rather than left implicit*: reject — exclude a designated value from the resolved set when its object key can't be derived, rather than pinning and rendering unverified foreign content. `objectKey` stays a nullable *column* (defensive; a historical row could still be null), but the resolver's producer contract is now "every newly resolved evidence entry has a non-null `objectKey`, or it isn't resolved at all."
