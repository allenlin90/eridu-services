...
- MCP list limits previously had no hard maximum; Phase 1 now caps both
  studio-wide list tools at 100.
- Bulk schedule schemas allow 1,000 items while the services intentionally
  execute per-item writes sequentially for partial success. T10 retained that
  cap after isolated maximum-size create/update measurements; request byte size
  remains independently bounded by `BODY_PARSER_LIMIT`.
...
- The former empty `openapi.module.ts` dynamic module had no providers or
  ...
  by `setupOpenAPI()` in `main.ts`. Its removal reduces the graph by one Nest
...
T9 updated `repository-pattern-nestjs` §6, `soft-delete-restore`,
`database-patterns` §1/§3, and closed the lazy-delegate row in
`erify-api-refactor-residuals.md`. T7 reconciled `service-pattern-nestjs`,
including the `BaseModelService` UID-generation contract.
...
visibility, and rollback. T12 accepted the result and reconciled the
persistence doctrine. Existing repositories are not migration debt by default;
migrate them only when touched and only when the matrix selects a simpler
boundary.
...
**Phase 3 result.** `ShowCatalogModule` owns show type, status, standard,
