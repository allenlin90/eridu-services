...
3. **Recorded Facts vs Analytical Rollups**: Facts that drive exception review, filtering, export, workflow gating, or performance dashboards are stored on the narrowest scoped table. Attendance state (`ON_TIME` / `LATE` / `MISSING`) and late minutes are **derived at read time** from `ShowCreator.actualStartTime`, the sticky `ShowCreator.attendanceMissing` marker, and `Show.startTime` — they are not stored as columns. Platform-scoped performance metrics with shipped workflows — **GMV, viewer count, CTR, and CTO** — live on `ShowPlatform` and feed [`show-performance-analytics.md`](../../../docs/features/show-performance-analytics.md). Likes, concurrents, cross-show aggregates, and future revenue semantics stay out of the task fact catalog until a concrete workflow and storage design promotes them.
   - **Promotion Workflow**: When a new analytical metric proves it has a concrete operational workflow depending on it (e.g., exception flag, mandatory review gate, dashboard filter), a follow-up design/ideation step decides whether to promote it to a typed indexed column on the narrowest entity or expose it through a read-model.
...
5. **Lean Core Models with Per-Fact Provenance**: Core models do not carry `actualsStatus` or row-level `actualsSource` columns. Time actuals store source in the model's existing `metadata` JSONB bucket: `metadata.actuals_source = { actual_start_time: 'MANAGER', actual_end_time: 'TASK' }`. Platform performance facts store template provenance in `ShowPlatform.metadata.performance_templates`. The actuals state is dynamically inferred from pair completeness: complete means both `actualStartTime` and `actualEndTime` are present; missing/incomplete means either edge is absent. Reads may project a "dominant source" for UI display, but stored provenance stays per fact.
...
model ShowPlatform {
  ...
  ctr                Decimal?                   @map("ctr") @db.Decimal(5, 2)
  cto                Decimal?                   @map("cto") @db.Decimal(5, 2)
...
2. **Snapshot Hydration (at generation)**: The generation engine scans the template's schema definitions for `system_fact_key` bindings:
   ...
   - Platform-scoped fields like `show_platform_actual_start_time`, `show_platform_gmv`, and `show_platform_view_count` undergo the same expansion, producing one input per assigned platform (e.g., `fld_actual_start_platform_<platformUid>`).
...
| Platform performance (GMV, viewer count, CTR, CTO) | dashboard summaries, trend inputs, and per-platform recorded values | `ShowPlatform(gmv, viewer_count, ctr, cto)` plus `metadata.performance_templates` provenance | `/studios/:id/performance`, show performance detail |
...
| `PerformanceMetricsWidget` | `/studios/:id/performance` dashboard summaries and trends | show Performance tab with platform/loop metrics | not used |
