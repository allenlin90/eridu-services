...
- `TALENT_MANAGER` is granted the same roster-write roles as `ADMIN`/`MANAGER` (add/reactivate/onboard/update/compensation read) because the role is expected to own day-to-day roster and compensation-default upkeep; `STUDIO_CREATOR_COMPENSATION_ROLES` (read) is kept in lockstep with the roster-write role set so no role can write compensation defaults it cannot read back.
