...
- No dedicated pending-resolution queue, status badges, or member-facing task-page indicator exist. The Shows-list filter is the accepted MVP discovery surface; revisit if studios report difficulty finding pending shows that way.
...
- The `toUpdate` restore path (a previously cancelled show reappearing in a republish) still writes `show_status_id` directly with no `Audit` row — the mirror-image gap of the `toRemove` bug fixed in this PR, just not yet fixed in the reappearance direction. See `docs/tech-debt/schedule-publish-restore-no-audit.md` for the confirmed restore policy and desired fix.
