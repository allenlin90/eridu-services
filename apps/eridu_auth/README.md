...
> **TLDR**: SSO authentication service built on [Better Auth](https://better-auth.com/) with PostgreSQL. Provides email/password login, JWT issuance (EdDSA/Ed25519), a JWKS endpoint, and OAuth/OIDC provider support for downstream clients such as Open WebUI. See [docs/](./docs/) for detailed guides.

**Current Status**: Email/password authentication, JWT/JWKS API auth, and OAuth/OIDC provider foundation
...
- ✅ **OAuth/OIDC Provider**: Downstream client login foundation for Open WebUI
...
| `/.well-known/openid-configuration` | GET | OIDC discovery metadata for OAuth clients |
| `/api/auth/oauth2/*`          | GET/POST | OAuth/OIDC provider endpoints      |
