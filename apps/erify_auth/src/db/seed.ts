  ...
  try {
    ...
    for (const testUser of TEST_USERS) {
      ...
      try {
        ...
        const mockRequest = new Request(`${env.BETTER_AUTH_URL}/api/auth/sign-up/email`, {
          method: 'POST',
          headers: {
        ...
        if (!response.ok) {
          ...
          throw new Error(`Failed to create user ${testUser.email}: ${errorData.message || response.statusText}`);
        }
    ...
    console.log('     Body: { "email": "test-user@example.com", "password": "testpassword123" }');
    console.log('');
    ...
    console.log('     The SDK will validate tokens using JWKS from /api/auth/jwks');
    console.log('');
