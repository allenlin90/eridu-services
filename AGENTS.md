...
- When picking up one accepted `docs/tech-debt/` entry for resolution, run the `tech-debt-delivery` skill, which executes [`.agents/workflows/tech-debt-delivery.md`](.agents/workflows/tech-debt-delivery.md) end-to-end (select, branch, fix, reconcile the register, verify, PR, `pr-ready` verdict). It is explicit-only and delivers a single item; `repository-health` still owns periodic reconciliation of the whole register.
...
- For large frontend route components (>200 LOC), see the `code-quality` skill (§ frontend quality reference).
- For frontend searchable controls, pagination, form contracts, refresh actions, and async lookup patterns, see `frontend-ui-components` (its table-view reference owns list and table surfaces) and `code-quality`.
...
[`code-quality` § backend testing](.agents/skills/code-quality/references/backend-testing-patterns.md#5-real-database-integration-tests)
...
- For `erify_api` module placement and persistence selection, load `erify-api-capability-refactoring` first. Use `nestjs-architecture` — its service, repository, and orchestration references — for the selected implementation's correctness rules.
