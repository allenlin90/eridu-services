...
- Store actual timestamps, performance facts, and revenue facts on the narrowest entity whose fact they describe: `Show` for overall event timing, `ShowCreator` for creator participation timing, `ShowPlatform` for platform stream/performance facts, and `StudioShiftBlock` for operator/member labor. Do not persist derived finance totals on operational tables; backend economics read models/calculators own reference figures, while frontend code may only preview local durations for input validation.
- For frontend money fields, compare dirty state and equality only after normalizing both the stored API decimal string and the user input with the relevant decimal/money helper. API strings may be decimal-equivalent without matching scale (for example, `20` vs `20.00`).
