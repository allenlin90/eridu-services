...
- Migration files must be generated by official tooling (Prisma for `erify_api`, Drizzle for `eridu_auth`; `better-auth` schema flow first for auth-domain changes). Do not hand-write new migrations. Manual SQL is allowed only as in-place customization with explicit comments and synced docs/skills updates.
