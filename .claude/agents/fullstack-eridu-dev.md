...
1. **Read the relevant skill** from `.agents/skills/<skill-name>/SKILL.md`. Common mappings:
   ...
   - Backend: `nestjs-architecture`, `erify-authorization`, `database-patterns`, `data-validation`, `engineering-best-practices-enforcer`
   - Analytics/JSONB: `jsonb-analytics-snapshot`
   - Multi-service: `nestjs-architecture` § orchestration
   ...
   - Full-stack: `frontend-ui-components` § table view
...
REST runtime. A provider module reused by MCP or a worker must not
