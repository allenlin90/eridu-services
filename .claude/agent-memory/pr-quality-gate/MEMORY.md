...
### Studio Show Management FE Patterns (feat/phase4-1e-show-management-design FE review, 2026-04-06)
- `useStudioShowStatusOptions` intentionally ignores the search string when calling `getShowStatuses`. Show statuses are a finite set; the backend endpoint does not support a name filter, so filtering is purely client-side via `filterOptions()`. The `name` param is intentionally omitted to avoid sending a param the backend would ignore.
- TEST BUG CONFIRMED: The test in `use-studio-show-form-lookup-options.test.tsx` asserts `getShowStatuses` is called with `{ name: 'live', limit: 20 }` after a search. This is WRONG — the implementation intentionally omits `name` from this call. The test is testing behavior that was deliberately not implemented. Fix: assert `{ limit: 10 }` (DEFAULT_LOOKUP_LIMIT only) for `showStatuses`, not `{ name, limit: 20 }`.
- `useDeleteStudioShow` is MISSING `queryClient.invalidateQueries({ queryKey: studioShowsKeys.listPrefix(studioId) })`. The mutation `onSuccess` only calls `removeQueries` on the detail key. The list table will not refresh after a delete. This is a real runtime bug — the deleted row stays in the table until the next page navigation or manual refresh.
- `useStudioSearchQuery` internal helper: correctly collects `search` state and includes it in the `queryKey`. `signal` forwarding from `queryFn` context to API fetchers is correct. All 7 exported lookup hooks use this shared helper correctly.
- Route structure: parent layout `shows.tsx` wraps `Outlet` with `StudioRouteGuard(routeKey="shows")`. Child page `shows/index.tsx` renders the full CRUD page. Pattern is correct — guard at layout level, page-shell at child level.
- Sidebar `shows` entry and `show-operations` entry are both gated on `hasStudioRouteAccess(role, 'shows')` from the same policy source. Route guard uses the same `routeKey="shows"`. Policy source is single-origin — CORRECT.
- `studioShowCreateFormSchema` uses `schedule_id: z.string().startsWith(UID_PREFIXES.SCHEDULE)` (schedule required). `studioShowEditFormSchema` uses `z.string().startsWith(UID_PREFIXES.SCHEDULE).or(z.literal(''))` (empty string allowed for orphan shows). Two-schema approach resolves the orphan-edit UX bug from prior reviews.
- `getStudioShow` hook in `use-studio-show.ts` does NOT export a `useStudioShow` mutation wrapper that's missing the `showLookupsKeys` invalidation. This is intentional — the delete mutation is the missing invalidation, not the detail query hook.
- `TABLE_OPTIONS.from` in `use-studio-show-management.ts` is `'/studios/$studioId/shows/'` (with trailing slash). This matches the child route path registered by TanStack Router. Correct.
