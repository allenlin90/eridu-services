...
> **Placement authority (2026-07):** For `erify_api`, `erify-api-capability-refactoring` is now authoritative for *placement* (which capability owns a use case, where a workflow/persistence lives) and supersedes the two skills below for that decision. Its persistence-decision matrix passed the `ShowStatus` pilot but remains **acceptance-gated** — repository-first persistence and the correctness rules in these skills stay canonical elsewhere until T12. "PRIMARY" below means canonical for persistence/correctness, not for module placement.
...
### When to Use Skills
- ✅ **Creating new `erify_api` capabilities/models** → `erify-api-capability-refactoring` first for placement, then `service-pattern-nestjs`, `repository-pattern-nestjs` for persistence/correctness (canonical outside the `ShowStatus` pilot until T12)
