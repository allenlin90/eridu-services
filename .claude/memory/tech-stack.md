...
- sonner 2.0.7 (toasts)
- usehooks-ts 3.1.1
- PWA shell via `vite-plugin-pwa`; Workbox SPA navigation fallback should bind to `/` rather than `index.html` on hosts that canonicalize `index.html`, and `/` must be included in the precache manifest via `templatedURLs: { '/': ['index.html'] }`
